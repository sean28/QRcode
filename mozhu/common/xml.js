function Parser(xmlString, useAttr) {
  ...
  Parser.prototype.readTag = function (tag) {
    ...
    while ((result = patt.exec(tag)) !== null) {
      if (result[1]) {
        nodeAttr['@' + result[1]] = this.transEntities(result[3]);
      } else if (result[5]) {
        nodeAttr['@' + result[5]] = this.transEntities(result[6]);
      }
    }
  ...
  Parser.prototype.addChildren = function (parent, child) {
    ...
    for (k in child) {
      if (parent[k]) {
        ...
        if(parent[k].length > 1){
          parent[k].splice(1, 0, child[k])
        }else{
  ...
  Parser.prototype.readText = function (nodeName) {
    ...
    while ((cdStart = this.xmlString.indexOf('<![CDATA[', start)) > -1 && cdStart < end) {
      text += this.transEntities(this.xmlString.substring(start, cdStart));
      cdEnd = this.xmlString.indexOf(']]>', cdStart);
      text += this.xmlString.substring(cdStart + 9, cdEnd);
      start = cdEnd + 3;
      end = this.xmlString.indexOf('</' + nodeName + '>', start);
    }
    text += this.transEntities(this.xmlString.substring(start, end));
  ...
  Parser.prototype.transEntities = function(str) {
  ...
  function xml2json(xmlString, useAttr) {
    var parser = new Parser(xmlString, !!useAttr);
    if (!parser.error) {
